...
class SpeakerBulkUpload extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  onValidate() {
    ...
    if (attendees.length !== 0 && this.state.eventValue) {
      this.setState({ loading: true });
       this.props.bulkValidateSpeaker(attendees);
      setTimeout(() => {
  ...
  onSubmit() {
    ...
    if (!this.state.csvFileRequired && !this.state.csvFileInvalid) {
      this.props.bulkUploadSpeaker(attendees, eventId);
      this.setState({ loading: true });
      setTimeout(() => {
        let message = "";
  ...
  Toaster(compRef, bulkUploadError) {
    compRef.setState({ loading: false });
    if (!bulkUploadError) {
      toast.success("Speaker uploaded successfully", {
...
const mapDispatchToProps = dispatch => {
  return {
     bulkUploadSpeaker: (attendees, eventId) =>
      dispatch(actions.bulkUploadSpeaker(attendees, eventId)),
     bulkValidateSpeaker: (attendees) =>
      dispatch(actions.bulkValidateSpeaker(attendees)),
